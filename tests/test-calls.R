...
####################################################################################
### test info pages, except some that don't work
####################################################################################

api <- .session$api()
resources <- names(api)
resources <- setdiff(resources, 
	c('download','library','metagenome','profile','project','sample','status'))
for (res in resources) call.MGRAST (res, 'info', issue=TRUE)
...
call.MGRAST('ma', 'fu', id=c(4447943.3, 4447192.3, 4447102.3, 4447103.3), 
	gr='level3', so='Sub', re='ab', iden=80, filter_l='phylum', filter='Firmicutes')
...
# this construction has to be automated, because things will change
#
# for (r in names(api)) {
# 	reqs <- setdiff(names(api[[r]]),'info')
# 	for (req in reqs) {
# 		s <- paste ('call.MGRAST(\'',strtrim(r,2),'\', \'',strtrim(req,2),'\')',collapse='',sep='')
# 		print(api[[r]][[req]]$example[1], quote=F)
# 		print(paste('###', s, collapse=''), quote=F)
# 	}
# 	cat('\n')
# }

# extract "examples" per resource, request
# tokenize
