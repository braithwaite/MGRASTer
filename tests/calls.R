...
call.MGRAST('ma', 'or', id=c(4447943.3, 4447192.3, 4447102.3, 4447103.3), 
	gro='family', so='Ref', resu='ab', ev=15, debug=TRUE, issue=issue)
...
call.MGRAST('ma', 'fu', id=c(4447943.3, 4447192.3, 4447102.3, 4447103.3), 
	gro='level3', so='Sub', resu='ab', iden=80, filter_l='phylum', filter='Firmicutes', debug=TRUE, issue=issue)
...
call.MGRAST('ma', 'fe', id=c(4447943.3, 4447192.3, 4447102.3, 4447103.3), 
	so='KE', resu='ev', len=25, debug=TRUE, issue=issue)

#------->GOT THIS FAR IN ACTUALLY CHECKING THESE AS LIVE CALLS

### http://api.metagenomics.anl.gov/metadata/template
call.MGRAST('metadata', 'te', debug=TRUE, issue=issue)
### http://api.metagenomics.anl.gov/metadata/cv
call.MGRAST('metadata', 'cv', debug=TRUE, issue=issue)
### http://api.metagenomics.anl.gov/metadata/export/mgp128
call.MGRAST('metadata', 'ex', 128, debug=TRUE, issue=issue)

### http://api.metagenomics.anl.gov/metagenome?limit=20&order=name
call.MGRAST('metageno','qu', li=20, ord='name', debug=TRUE, issue=issue)
### http://api.metagenomics.anl.gov/metagenome/mgm4447943.3?verbosity=metadata
call.MGRAST('metageno','ins', 4447943.3, ve='me', debug=TRUE, issue=issue)

### http://api.metagenomics.anl.gov/project?limit=20&order=name
call.MGRAST('proj', 'qu', li=20, or='na', debug=TRUE, issue=issue)
### http://api.metagenomics.anl.gov/project/mgp128?verbosity=full
call.MGRAST('proj', 'ins', 128, ve='fu', debug=TRUE, issue=issue)

### http://api.metagenomics.anl.gov/sample?limit=20&order=name
call.MGRAST('sa', 'qu', li=20, or='na', debug=TRUE, issue=issue)
### http://api.metagenomics.anl.gov/sample/mgs25823?verbosity=full
call.MGRAST('sa', 'ins', 25823, ve='fu', debug=TRUE, issue=issue)

### http://api.metagenomics.anl.gov/validation/template/
call.MGRAST('va', 'te', debug=TRUE, issue=issue)
### http://api.metagenomics.anl.gov/validation/data/?template=
call.MGRAST('va', 'da', debug=TRUE, issue=issue)
